function [retrieved_indexes, similarities, new_case] = retrieve(case_library, new_case, threshold)
    ...
    %holiday_type , price , number_persons region transportation duration
    %season accomodation
    weighting_factors = [3 5 4 4 2 2 5 1];
    ...
    for i=1:size(case_library,1)
        ...
        DG = (distances * transpose(weighting_factors)) / sum(weighting_factors);
...
function [holiday_type_sim] = get_holiday_type_similarities()
    ...
    holiday_type_sim.similarities = [
        ...
        1.0         0.6   0.1   0.2        0.4      0.5     0.1    0.2;  % Active
        0.6         1.0   0.2   0.1        0.3      0.6     0.2    0.1;  % Bathing
        0.6         0.2   1.0   0.3        0.2      0.1     0.4    0.1;  % City
        0.4         0.1   0.7   1.0        0.8      0.2     0.3    0.1;  % Education
        0.3         0.3   0.2   0.8        1.0      0.4     0.1    0.2;  % Language
        0.2         0.6   0.5   0.4        0.6      1.0     0.2    0.3;  % Recreation
        0.8         0.2   0.1   0.2        0.1      0.2     1.0    0.5;  % Skiing
        0.2         0.1   0.3   0.1        0.1      0.3     0.4    1.0;  % Wandering
    ];
end
...
function [accommodation_sim] = get_accommodation_similarities()
    ...
    accommodation_sim.similarities = [
        ...
        1.0         0.8  0.5  0.2  0.7  0.6;  % FiveStars
        0.8         1.0  0.4  0.1  0.6  0.5;  % FourStars
        0.5         0.4  1.0  0.3  0.8  0.7;  % HolidayFlat
        0.2         0.1  0.3  1.0  0.4  0.2;  % OneStar
        0.7         0.6  0.8  0.4  1.0  0.9;  % ThreeStars
        0.6         0.5  0.7  0.2  0.9  1.0;  % TwoStars
    ];
end
...
function [res] = calculate_local_distance(sim, val1, val2)
    ...
    if isempty(i1) || isempty(i2)
        error('Categoria não encontrada na matriz de similaridades.');
    end

    % Obtenha a similaridade correspondente
    similarity = sim.similarities(i1, i2);

    % Calcule a distância local
