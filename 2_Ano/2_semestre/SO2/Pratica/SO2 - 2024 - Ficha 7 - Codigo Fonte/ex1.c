...
#define SIZEMSG 256 * sizeof(TCHAR),
...
int WINAPI _tWinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPTSTR lpCmdLine, int nCmdShow) {
  ...
  wcApp.hbrBackground = CreateSolidBrush(RGB(0, 0, 0));//(HBRUSH)GetStockObject(WHITE_BRUSH); 
  ...
  // ============================================================================
	...
	TEXT("NÃO SOU MALUKITO!"),// Texto que figura na barra do título
	WS_OVERLAPPEDWINDOW,	// Estilo da janela (WS_OVERLAPPED= normal)
	200,		// Posição x pixels (default=à direita da última)
	200,		// Posição y pixels (default=abaixo da última)
	120,		// Largura da janela (em pixels)
	150,		// Altura da janela (em pixels)
...
DWORD WINAPI MovitMovit(LPVOID lParam) {
...
LRESULT CALLBACK TrataEventos(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) {	
	...
	static int salto;
	static int limDir;
	HANDLE hThr=NULL, hMutex=NULL;

	static DadosThread dT;
	
	
	switch (messg) {	

	case WM_CREATE:
		hBmp = (HBITMAP) LoadImage(NULL, TEXT("lol24.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
	  ...
	  case WM_PAINT:
		  ...
		  HBITMAP backupbuffer = CreateCompatibleBitmap(hdc, width, height);

		  int savedDC = SaveDC(backbuffDC);
		  SelectObject(backbuffDC, backupbuffer);
	  ...
	  case WM_CLOSE:
		  ...
		  if (MessageBox(hWnd, TEXT("NÃO ES MALUKXITO?!?!!"), TEXT("rIaS"), MB_YESNO | MB_ICONQUESTION) == IDNO) {
